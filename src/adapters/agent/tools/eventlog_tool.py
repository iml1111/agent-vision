...
_eventlog_adapter = None


def set_eventlog_adapter(adapter):
    """Set the EventLog adapter for tool use"""
    global _eventlog_adapter
    _eventlog_adapter = adapter
...
async def funnel_analysis(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    if _eventlog_adapter is None:
        return {
            "content": [{"type": "text", "text": "EventLog adapter not initialized"}],
            "is_error": True
        }
    ...
    try:
        # Build MongoDB aggregation pipeline for funnel analysis
        match_stage = {
            "$match": {
        ...
        # Group by user and collect events
        group_stage = {
            "$group": {
                "_id": "$user_id",
        ...
        results = await _eventlog_adapter.aggregate(pipeline)
...
async def retention_analysis(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        period_unit = args.get("period_unit", "day")

        # Simplified retention query
        pipeline = [
            {
                "$match": {
...
async def segment_analysis(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for segment in results[:20]:  # Top 20 segments
